...
/* Globals */
static struct rpmsg_endpoint lept = {.dest_addr = RPMSG_ADDR_ANY}; 
			     ...
			     uint32_t src, void *priv)
{
	...
	(void)src;
	(void)priv;
	LPRINTF("received echo data number %lu of size %lu \r\n\r\n",
	...
	for (i = 0; i < (int)r_payload->size; i++) {
		if (r_payload->data[i] != 0xA5) {
			LPERROR("Data corruption at index %d\r\n", i);
			err_cnt++;
			break;
		}
	}
...
static int app(struct rpmsg_device *rdev, void *priv)
{
    ...
    /* Wait until the other core becomes ready */
    ...
    /* Send a message for shutdown */
    size = PAYLOAD_MIN_SIZE;
	memset(&(i_payload->data[0]), SHUTDOWN_PAYLOAD, size);
	LPRINTF("sending a message for shutdown\r\n");
	ret = rpmsg_send(&lept, i_payload,
			 (2 * sizeof(unsigned long)) + size);
	if (ret < 0) {
		LPERROR("Failed to send the message for shutdown...\r\n");
	}
...
void TaskRPMSG(void *pvParameters)
{
	...
	/* Initialize platform */
	ret = platform_init(1, NULL, &platform);
	if (ret) {
