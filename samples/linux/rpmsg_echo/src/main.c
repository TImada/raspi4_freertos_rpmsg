                 ...
                 uint32_t src, void *priv)
{
    ...
    if (payload[offset] == SHUTDOWN_PAYLOAD) {
        LPRINTF("Received a shutdown message\n");
        shutdown_req = 1;
        goto destroy_ept;
    }

    /* Send data back MSG_LIMIT time to master */
    LPRINTF("received data from FreeRTOS over RPMSG, size = %lu\n", len);
    LPRINTF("echo message number %u\n\n", (unsigned int)count);
    count++;
